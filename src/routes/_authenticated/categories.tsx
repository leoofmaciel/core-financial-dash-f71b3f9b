...
function CategoriesPage() {
  ...
  const seedDefaults = async () => {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return;
    const rows = defaults.map((n) => ({ user_id: user.id, name: n, color: "#1e40af" }));
        ...
        <div className="flex gap-2">
          ...
          <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) { setName(""); setEditing(null); } }}>
            <DialogTrigger asChild>
              <Button onClick={() => { setEditing(null); setName(""); setColor("#1e40af"); }}><Plus className="h-4 w-4 mr-1" /> Nova categoria</Button>
            </DialogTrigger>
            <DialogContent>
          ...
          <Card key={c.id}>
            <CardContent className="p-4 flex items-center justify-between gap-2">
              ...
              <div className="flex gap-1">
                <Button size="icon" variant="ghost" onClick={() => { setEditing(c); setName(c.name); setColor(c.color); setOpen(true); }}><Pencil className="h-4 w-4" /></Button>
                <AlertDialog>
